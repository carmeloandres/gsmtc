...
// Comments
function gsmtc_reply() {

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'gsmtc_reply' );


// Comments
if ( ! function_exists( 'gsmtc_comment' ) ) :
    /**
     * Template for comments and pingbacks.
     *
     * Used as a callback by wp_list_comments() for displaying the comments.
     */
    function gsmtc_comment( $comment, $args, $depth ) {
...
<li id="comment-<?php comment_ID(); ?>" <?php comment_class( 'media' ); ?>>
    <div class="comment-body">
        <?php _e( 'Pingback:', 'gsmtc' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'gsmtc' ), '<span class="edit-link">', '</span>' ); ?>
    </div>
        ...
        <div class="col comment-content">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mt-0"><?php printf( __( '%s <span class="says d-none">says:</span>', 'gsmtc' ), sprintf( '<h3 class="">%s</h3>', get_comment_author_link() ) ); ?></div>
                    <small>
                        <div class="comment-meta text-muted">

                            <time datetime="<?php comment_time( 'c' ); ?>">
                                <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'gsmtc' ), get_comment_date(), get_comment_time() ); ?>
                            </time>

                            <?php edit_comment_link( __( 'Edit', 'gsmtc' ), '<span class="edit-link">', '</span>' ); ?>
                        </div>
                    </small>
                    ...
                    <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'gsmtc' ); ?></p>
                    <?php endif; ?>
...
endif; // ends check for gsmtc_comment()
...
function wp44138_change_comment_form_cookies_consent( $fields ) {
    ...
    $fields['cookies'] = '<p class="comment-form-cookies-consent custom-control form-check mb-3">' .
        ...
        '<label for="wp-comment-cookies-consent" class="form-check-label">' . __( 'Save my name, email, and website in this browser for the next time I comment.', 'gsmtc' ) . '</label>' .
        '</p>';
    return $fields;
}
